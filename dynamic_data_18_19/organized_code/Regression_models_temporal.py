...
"""
0. 
One-hot encoding the days of the week
"""

# data_processed = pd.get_dummies(data_processed["weekday"],prefix='day',drop_first=True)
data_processed = pd.concat([data_processed,pd.get_dummies(data_processed['weekday'], prefix='day')],axis=1)
...
"""
X = data_processed[['Speed_mean', 'Speed_median', 'Speed_min', 'Speed_max', 'Speed_std', 'ROT_mean', 'ROT_median', 'ROT_min', 'ROT_max', 'ROT_std', 'day_1', 'day_2', 'day_3', 'day_4', 'day_5', 'day_6', 'day_0']]
...
feature_names = ['Speed_mean', 'Speed_median', 'Speed_min', 'Speed_max', 'Speed_std', 'ROT_mean', 'ROT_median', 'ROT_min', 'ROT_max', 'ROT_std', 'day_1', 'day_2', 'day_3', 'day_4', 'day_5', 'day_6', 'day_0']
...
### RESULTS:
# Best parameters: {'ccp_alpha': 2, 'max_depth': 15, 'max_features': 5, 'n_estimators': 90}
# Mean cross-validated score of the best_estimator: 0.81
#%%
"""
2.
Random Forest model
...
print("Random Forest Training set score: {:.3f}".format(regr.score(X_train_scaled, y_train)))
...
grid_search2 = GridSearchCV(regr2, param_grid2, cv=5, n_jobs =3)
grid_search2.fit(X_valid, y_valid) #on validation set
print("Best parameters: {}".format(grid_search2.best_params_))
best_parameters2 = grid_search2.best_params_
print("Mean cross-validated score of the best_estimator: {:.3f}".format(grid_search2.best_score_))
...
print("Gradient Boosting Training set score: {:.3f}".format(regr2.score(X_train, y_train)))
print("Gradient Boosting Test set score: {:.3f}\n".format(regr2.score(X_test, y_test)))
...
print("Neural Network Training set score: {:.3f}".format(nn.score(X_train_scaled, y_train)))
print("Neural Network Test set score: {:.3f}\n".format(nn.score(X_test_scaled, y_test)))

#mean absolute error
y_pred = nn.predict(X_test_scaled)
MAE_NN = mean_absolute_error(y_test, y_pred)
print("Absolute mean error for NN is {:.3f}m\n".format(MAE_NN))


#Cross validation - takes a while, is training 5 neural nets
cross_val_scores_nn = cross_val_score(nn, X =X_valid_scaled, y =y_valid, cv = 5)
print("Cross-validation scores for linear regression:\n {}\n".format(cross_val_scores_nn))
print("Mean cross validation for LR {:.3f}".format(np.mean(cross_val_scores_nn)))

###RESLUTS:
# Neural Network Training set score: 0.834
# Neural Network Test set score: 0.818

# Absolute mean error for NN is 20.467m

####RESULTS with Other ship types
# Neural Network Training set score: 0.79
# Neural Network Test set score: 0.77

# Absolute mean error for NN is 22.135m
#%%
"""
3. Neural Network
...
rf_predicted_tanker_y = regr2.predict(tanker_X)
...
print("GB tanker score: {:.3f}".format(regr2.score(tanker_X, tanker_y)))
print("mean length of a Tanker is {:.3f}m".format(np.mean(tanker_y)))
print("max length of a tanker_y vessel is {:.3f}m".format(np.max(tanker_y)))
print("min length of a tanker_y vessel is {:.3f}m\n".format(np.min(tanker_y)))


rf_predicted_cargo_y = regr2.predict(cargo_X)
...
print("GB cargo score: {:.3f}".format(regr2.score(cargo_X, cargo_y)))
print("mean length of a cargo vessel is {:.3f}m".format(np.mean(cargo_y)))
print("max length of a cargo vessel is {:.3f}m".format(np.max(cargo_y)))
print("min length of a cargo vessel is {:.3f}m\n".format(np.min(cargo_y)))

rf_predicted_passenger_y = regr2.predict(passenger_X)
...
print("GB passenger score: {:.3f}".format(regr2.score(passenger_X, passenger_y)))
print("mean length of a passenger vessel is {:.3f}m".format(np.mean(passenger_y)))
print("max length of a passenger_y vessel is {:.3f}m".format(np.max(passenger_y)))
print("min length of a passenger_y vessel is {:.3f}m\n".format(np.min(passenger_y)))


rf_predicted_support_y = regr2.predict(support_X)
...
print("GB support score: {:.3f}".format(regr2.score(support_X, support_y)))
print("mean length of a support vessel is {:.3f}m".format(np.mean(support_y)))
print("max length of a support_y vessel is {:.3f}m".format(np.max(support_y)))
print("min length of a support_y vessel is {:.3f}m\n".format(np.min(support_y)))

rf_predicted_fast_ferry_y = regr2.predict(fast_ferry_X)
...
print("GB fast ferry score: {:.3f}".format(regr2.score(fast_ferry_X, fast_ferry_y)))
...
rf_predicted_fishing_y = regr2.predict(fishing_X)
...
print("GB Fishing score: {:.3f}".format(regr2.score(fishing_X, fishing_y)))
print("mean length of a fishing is {:.3f}m".format(np.mean(fishing_y)))
print("max length of a fishing_y vessel is {:.3f}m".format(np.max(fishing_y)))
print("min length of a fishing_y vessel is {:.3f}m\n".format(np.min(fishing_y)))

rf_predicted_boat_y = regr2.predict(boat_X)
...
print("GB boat score: {:.3f}".format(regr2.score(boat_X, boat_y)))
print("mean length of a boat is {:.3f}m".format(np.mean(boat_y)))
print("max length of a boat_y vessel is {:.3f}m".format(np.max(boat_y)))
print("min length of a boat_y vessel is {:.3f}m\n".format(np.min(boat_y)))
